...
$result= mysqli_fetch_array($ex1);
  	$ngay_hh=strtotime($result['ngay_hh']);	
	$ngay_ht=strtotime(date('Y-m-d'));			
	$kq2=($ngay_hh -$ngay_ht)/(24*60*60);

$qrpt= mysqli_query($connect,"select count(ma_bai_dang) as tong from sanphamgiaicuu where trang_thai=1 and'".($kq2>0)."'");
...
<table class="table">
	<tr>
		<td><label>Kết quả tìm kiếm cho sản phẩm đã hết hạn là: </label></td>
	</tr>
<?php 
	// $search= $_GET['search'];
	// include('connect.php');
	$sl= "select * from sanphamgiaicuu where trang_thai='1' and'".($kq2>0)."' LIMIT $start, $limit";
			...
			<div style="height:300px; padding-bottom: 15px;" class="sp img-thumbnail col-xs-12 col-sm-6 col-md-12 col-lg-12">
				...
				<div style="padding-top: 10px; margin-right: 20px; margin-left: 20px;" class="text-left col-sm-6 col-md-6">
					<div class="title col-xs-12 col-sm-12 col-md-12 col-lg-12">
						...
						<span>Số lượng :<?php echo $row['so_luong']." Kg"; ?></span><br>
    ...
    // ngược lại hiển thị thẻ a
				if ($i == $trang){
					...
					echo '<a href="index.php?menu=timkiemsphh&page='.$i.'">'.$i.'</a> ';
				}
			} ?></li>
