...
<table class="table">
	<tr>
		<td><label>Kết quả tìm kiếm cho sản phẩm gần hết hạn là: </label></td>
	</tr>
<?php 
	// $search= $_GET['search'];
	// include('connect.php');
			...
			<div style="height:300px; padding-bottom: 15px;" class="sp img-thumbnail col-xs-12 col-sm-6 col-md-12 col-lg-12">
				...
				<div style="padding-top: 10px; margin-right: 20px; margin-left: 20px;" class="text-left col-sm-6 col-md-6">
					<div class="title col-xs-12 col-sm-12 col-md-12 col-lg-12">
						...
						<span>Mô tả sản phẩm :<?php echo $row['mo_ta']; ?></span><br>
					</div>
	...
	<ul class="pagination" >
		<?php  if ($trang > 1 && $tong_record > 1){?>
			<li><a href="index.php?menu=spganhh&page=<?php echo $trang-1?>">&laquo;</a></li><?php } ?>
    ...
    // ngược lại hiển thị thẻ a
				if ($i == $trang){
					...
					echo '<a href="index.php?menu=spganhh&page='.$i.'">'.$i.'</a> ';
				}
			} ?></li>
			<?php if ($trang < $tong_record && $tong_record > 1){ ?>
				<li><a href="index.php?menu=spganhh&page=<?php echo $trang+1?>">&raquo;</a></li><?php } ?>
			</ul>
		</div>