...
<?php 
$qrpt= mysqli_query($connect,"select count(ma_bai_dang) as tong from sanphamgiaicuu where ten_sp like '%dưa hấu%'");
...
<table class="table">
	<tr>
		<td><label>Kết quả tìm kiếm cho từ khóa dưa hấu là: </label></td>
	</tr>
<?php 
	// $search= $_GET['search'];
	// include('connect.php');
	$sl= "select * from sanphamgiaicuu where ten_sp like '%dưa hấu%' LIMIT $start, $limit";
	...
	$num= mysqli_num_rows($exec);
		...
		if($num==0){
			?>
		<tr>
			<td>Không tìm thấy kết quả cho từ khóa dưa hấu</td>
		</tr>
			<?php
		}	
    ...
    // ngược lại hiển thị thẻ a
				if ($i == $trang){
					...
					echo '<a href="index.php?menu=timkiem_duahau&page='.$i.'">'.$i.'</a> ';
				}
			} ?></li>
